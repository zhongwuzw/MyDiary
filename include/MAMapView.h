...
@interface MAMapView(Coordinate)

-(CGPoint)CoordinateToScreenXY:(CLLocationCoordinate2D)coordiante;
-(CLLocationCoordinate2D)ScreenXYToCoordinate:(CGPoint)screenXY;
@end
...
/**
 *返回从code频道申请的用户Key 
 *通过这个函数返回用户Key给MAMapKit，底层引擎会根据用户返回的Key 来发起坐标偏移查询，以此来纠正表示用户位置的蓝点在地图上的位置偏移，如果没有实现此回调函数，或者返回的Key有错误，那么蓝点的位置将不会被纠正
 
 说明:目前搜索服务暂时不需要传入Key,所以不实现这个回调函数也可以进行坐标偏移。但是AMapSDK 以后的版本会加进Key鉴权,所以头文件中保留这个函数。
 */
